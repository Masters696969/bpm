const API_URL = "backend/admin_timesheet_data.php";
...
function bindEvents() {
    ...
    if (periodSelect) {
        periodSelect.addEventListener("change", async function () {
            ...
            if (currentPeriodId) {
                await loadPeriodData(currentDepartmentId, currentPeriodId);
            }
        });
    }
    ...
    if (btnRefresh) {
        btnRefresh.addEventListener("click", async () => {
            if (!currentDepartmentId || !currentPeriodId) return;
            await loadPeriodData(currentDepartmentId, currentPeriodId);
        });
    }
    ...
    if (btnRecompute) {
        btnRecompute.addEventListener("click", async () => {
            ...
            const confirm = await Swal.fire({
                ...
                text: "This will rebuild the selected department period summary from timesheet_daily records.",
                icon: "question",
            ...
            fd.append("department_id", currentDepartmentId);
    ...
    if (btnPublishNow) {
        btnPublishNow.addEventListener("click", async () => {
            ...
            const confirm = await Swal.fire({
                title: "Publish now?",
                text: "The selected timesheet period will be finalized immediately.",
                icon: "question",
                showCancelButton: true,
            ...
            try {
                ...
                await Swal.fire("Published", data.message, "success");
...
async function loadDepartments() {
    try {
        const data = await fetchJson(`${API_URL}?action=get_departments`);
        ...
        currentDepartmentId = data.departments[0].department_id;
        updateDepartmentHeader(data.departments[0].department_name);
        await loadPeriods();
    } catch (err) {
        Swal.fire("Error", err.message, "error");
    }
...
async function loadPeriods(keepSelection = false) {
    try {
        if (!currentDepartmentId) return;

        const data = await fetchJson(`${API_URL}?action=get_periods&department_id=${encodeURIComponent(currentDepartmentId)}`);
        if (!data.success) throw new Error(data.message || "Failed to load periods.");

        const select = document.getElementById("periodSelect");
        if (!select) return;

        const oldValue = keepSelection ? currentPeriodId : null;
        select.innerHTML = "";
        ...
        if (!data.periods || data.periods.length === 0) {
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="12" style="text-align:center; padding:30px;">
                            No timesheet periods found for this department.
                        </td>
                    </tr>
                `;
            }
            ...
            renderStats({ employees: 0, ot_hours: 0, late_minutes: 0, issues: 0 });
            renderIssues([]);
            return;
        }
        ...
        data.periods.forEach((period) => {
            ...
            if (
                (oldValue && String(oldValue) === String(period.period_id)) ||
                (!oldValue && data.periods[0].period_id === period.period_id)
            ) {
...
async function loadPeriodData(departmentId, periodId) {
    try {
        const data = await fetchJson(
            `${API_URL}?action=get_period_data&department_id=${encodeURIComponent(departmentId)}&period_id=${encodeURIComponent(periodId)}`
        );

        if (!data.success) throw new Error(data.message || "Failed to load timesheet data.");

        renderPeriodHeader(data.period);
        updateDepartmentHeader(data.period.department_name || "Unknown Department");
...
function updateDepartmentHeader(deptName) {
    const el = document.getElementById("selectedDepartmentLabel");
    if (el) {
        el.innerHTML = `
            <i data-lucide="building-2" style="width:14px;"></i>
            ${escapeHtml(deptName || "Unknown Department")}
        `;
    }
}
...
async function viewLogs(employeeId) {
    try {
        const data = await fetchJson(
            `${API_URL}?action=employee_logs&department_id=${encodeURIComponent(currentDepartmentId)}&period_id=${encodeURIComponent(currentPeriodId)}&employee_id=${encodeURIComponent(employeeId)}`
        );
