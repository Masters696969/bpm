...
// -------------------------------------------------------------
// Merit Matrix Endorsed Proposals Logic
// -------------------------------------------------------------
async function fetchEndorsedProposals() {
    ...
    try {
        const response = await fetch('be_meritmatrixmgt.php?action=fetch_endorsed_proposals');
        const res = await response.json();

        if (res.success) {
            if (res.data.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6"><div class="empty-state"><p>No pending matrix proposals to review.</p></div></td></tr>`;
                return;
            }
            ...
            tbody.innerHTML = res.data.map(req => {
                ...
                <tr>
                    ...
                    <td><span class="type-pill" style="background:rgba(139,92,246,0.1); color:#8b5cf6;"><i data-lucide="trending-up"></i> Merit Matrix</span></td>
                    ...
                    <td>
                        <button class="btn-review" onclick="viewMeritProposal('${req.BatchReference}', '${encodeURIComponent(req.Reason)}')">
...
window.viewMeritProposal = async function (batchRef, reasonStr) {
    ...
    try {
        const response = await fetch(`be_meritmatrixmgt.php?action=fetch_proposal_details&batch_reference=${batchRef}`);
        const res = await response.json();
        if (res.success) {
            tbody.innerHTML = res.data.map(req => `
                <tr>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnEndorseProposal')?.addEventListener('click', async () => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch('be_meritmatrixmgt.php', {
                    method: 'POST',
    ...
    document.getElementById('btnRejectProposal')?.addEventListener('click', async () => {
        if (!activeBatchRef) return;
        const result = await Swal.fire({
            title: 'Reject Merit Matrix Proposal?',
