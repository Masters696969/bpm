...
async function fetchClaimsData() {
    const res = await fetch("backend/claims_data.php", {
        method: "GET",
...
function renderSummary(summary) {
    countForApproval.textContent = summary.for_approval ?? summary.pending ?? 0;
...
function buildRow(row) {
    ...
    tr.innerHTML = `
        ...
        <td>
            ...
            <div class="subtext">${escapeHtml(row.Description || "-")}</div>
        </td>
        <td>
...
function openModal(data) {
    ...
    const notesBox = document.getElementById("mOfficerNotes");
    if (notesBox) {
        const submissionNotesText =
            data.EmployeeNotes ||
            data.SubmissionNotes ||
            data.OfficerNotes ||
            "No submission notes.";
        notesBox.textContent = submissionNotesText;
    }

    document.getElementById("hrRemarks").value = data.AdminNotes || data.HRNotes || "";

    const proofBox = document.getElementById("mProofBox");
    if (data.ReceiptImage && String(data.ReceiptImage).trim() !== "") {
        const imgSrc = "../../" + String(data.ReceiptImage).replace(/^\/+/, "");
        proofBox.innerHTML = `
...
function bindEvents() {
    ...
    resetFiltersBtn.addEventListener("click", () => {
        ...
        tabs.forEach((t) => t.classList.remove("active"));
        document.querySelector('.tab-btn[data-status="PENDING"]')?.classList.add("active");
    ...
    approveBtn.addEventListener("click", async () => {
        ...
        const result = await Swal.fire({
            ...
            icon: "question",
            title: "Approve claim?",
            text: "This will approve the claim at Admin level.",
        ...
        try {
            ...
            await Swal.fire({
                ...
                icon: "success",
                title: "Approved",
                text: "Claim has been approved by Admin."
            });
    ...
    rejectBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            await Swal.fire({
                ...
                icon: "success",
                title: "Rejected",
                text: "Claim has been rejected by Admin."
            });
