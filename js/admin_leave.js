...
async function fetchLeaveData() {
    const res = await fetch("backend/admin_leave_data.php", {
        method: "GET",
...
function buildRow(row) {
    ...
    const hasPreviousReview = !!(row.OfficerApprovedByName || row.OfficerNotes);
    ...
    tr.innerHTML = `
        ...
        <td>
            ...
            <div class="subtext">${hasPreviousReview ? "Previously reviewed" : "Direct admin review"}</div>
        </td>
...
function openModal(data) {
    ...
    document.getElementById("mLeaveCategory").textContent =
        Number(data.IsPaid) === 1 ? "Paid Leave" : "Unpaid Leave";
    ...
    document.getElementById("mTotalDays").textContent =
    ...
    document.getElementById("mOfficerApprovedBy").textContent =
        data.OfficerApprovedByName || "No prior officer review";

    document.getElementById("mOfficerRemarksText").textContent =
        data.OfficerNotes || "No prior remarks provided.";
    ...
    if (Number(data.IsPaid) === 1) {
        ...
        document.getElementById("mTotalCredits").textContent =
            Number(data.TotalCredits || 0).toFixed(2);
        document.getElementById("mUsedCredits").textContent =
            Number(data.UsedCredits || 0).toFixed(2);
        document.getElementById("mRemainingCredits").textContent =
            Number(data.RemainingCredits || 0).toFixed(2);
    } else {
    ...
    const canAct =
        data.Status === "PENDING" || data.Status === "APPROVED_BY_OFFICER";
    ...
    approveBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            await Swal.fire({
                ...
                icon: "success",
                title: "Approved",
                text: "Request has been approved by admin."
            });
    ...
    rejectBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            await Swal.fire({
                ...
                icon: "success",
                title: "Rejected",
                text: "Request has been rejected by admin."
            });
