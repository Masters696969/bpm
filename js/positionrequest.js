...
// Notification Dropdown Logic (Matching cycle.js style)
function initNotifications() {
    ...
    // Mark all as read (Mock action since these are dynamic requests)
    ...
    if (markReadBtn) {
        markReadBtn.addEventListener('click', (e) => {
            ...
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'info',
                title: 'Notifications hidden for this session.',
                showConfirmButton: false,
                timer: 2000
            });
        });
...
// Helper for SweetAlert2 Action Confirmations
window.confirmAction = async (title, text, icon, confirmText, formId) => {
    const result = await Swal.fire({
        title: title,
        text: text,
        icon: icon,
        showCancelButton: true,
        confirmButtonColor: icon === 'error' || icon === 'warning' ? '#ef4444' : '#2ca078',
        cancelButtonColor: '#6b7280',
