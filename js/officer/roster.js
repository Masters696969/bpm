  ...
  state.shifts.forEach((shift) => {
    ...
    btn.innerHTML = `
      ...
      <span class="shift-meta">
        ...
        <small>${escapeHtml(shift.break_label || "No break")}</small>
        ${shift.grace_label ? `<small>${escapeHtml(shift.grace_label)}</small>` : ``}
      </span>
    `;
  ...
  if (leave) {
    ...
    content = `
      <span class="shift-pill ${escapeHtml(assignment.class || "shift-custom")}"
            title="Break: ${escapeHtml(assignment.break_label || "No break")} | Grace: ${escapeHtml(assignment.grace_label || "0 hr grace")}">
        ${escapeHtml(assignment.label || assignment.shift_code || "Set")}
      </span>
      ${assignment.break_label ? `<small>${escapeHtml(assignment.break_label)}</small>` : ``}
      ${assignment.grace_label ? `<small>${escapeHtml(assignment.grace_label)}</small>` : ``}
    `;
  } else {
...
function assignShift(employeeId, date, shift, source = "manual") {
  ...
  state.assignments[key] = {
    ...
    // new display fields
    break_hours: Number(shift.break_hours || 0),
    grace_hours: Number(shift.grace_hours || 0),
    break_label: shift.break_label || "No break",
    grace_label: shift.grace_label || "0 hr grace",
