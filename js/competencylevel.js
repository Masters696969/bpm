document.addEventListener("DOMContentLoaded", () => {
    ...
    // 6. Competency Level CRUD Logic
    const addLvlBtn = document.getElementById("addLevelBtn");
    ...
    const toggleLevelModal = (modal, show) => {
        if (!modal) return;
        if (show) {
    ...
    if (addLvlBtn) addLvlBtn.addEventListener("click", () => toggleLevelModal(addModal, true));
    [closeAddModal, cancelAdd].forEach(btn => {
        if (btn) btn.addEventListener("click", () => toggleLevelModal(addModal, false));
    });
    ...
    if (addForm) {
        addForm.addEventListener("submit", async (e) => {
            ...
            try {
                const response = await fetch("backend/level_action.php", { method: "POST", body: formData });
    ...
    // Edit Level
    document.querySelectorAll(".edit-btn-lvl").forEach(btn => {
        btn.addEventListener("click", () => {
            document.getElementById("edit_lvl_id").value = btn.dataset.id;
            document.getElementById("edit_lvl_name").value = btn.dataset.name;
            document.getElementById("edit_lvl_rank").value = btn.dataset.rank;
            document.getElementById("edit_lvl_desc").value = btn.dataset.desc;
            toggleLevelModal(editModal, true);
        });
    });

    [closeEditModal, cancelEdit].forEach(btn => {
        if (btn) btn.addEventListener("click", () => toggleLevelModal(editModal, false));
    });
    ...
    // Delete Level
    document.querySelectorAll(".delete-btn-lvl").forEach(btn => {
        btn.addEventListener("click", () => {
            const id = btn.dataset.id;
            Swal.fire({
                title: "Are you sure?",
                text: "Removing this level cannot be undone.",
                icon: "warning",
    ...
    window.addEventListener("click", (e) => {
        if (e.target.classList.contains("modal-overlay-lvl")) toggleLevelModal(e.target, false);
    });
