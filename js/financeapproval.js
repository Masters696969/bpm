document.addEventListener("DOMContentLoaded", () => {
    ...
    const apiUrl = 'financeapproval_action.php';
    ...
    const statusBadge = (status) => {
        ...
        if (s === 'finance approved') {
            return `<span class="badge-premium badge-success" style="background: rgba(44, 160, 120, 0.1); color: var(--brand-green);"><i data-lucide="shield-check"></i> Finance Approved</span>`;
        }
        if (s === 'archived') {
            return `<span class="badge-premium badge-secondary"><i data-lucide="archive"></i> Archived</span>`;
        }
        if (s.includes('pending approval') || s.includes('approved')) {
            return `<span class="badge-premium badge-warning"><i data-lucide="clock"></i> Pending Approval</span>`;
        }
    ...
    const renderBatches = (batches, targetBody) => {
        ...
        batches.forEach(b => {
            ...
            const isReady = s.includes('pending approval') || s === 'approved';
            const isHistory = ['disbursed', 'archived', 'finance approved'].includes(s);
