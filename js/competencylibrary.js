document.addEventListener("DOMContentLoaded", () => {
    ...
    // 6. Competency Accordion Logic
    const competencyHeaders = document.querySelectorAll(".competency-header");
    competencyHeaders.forEach(header => {
        header.addEventListener("click", () => {
            const card = header.closest(".competency-card");
            card.classList.toggle("active");
            if (typeof lucide !== "undefined") lucide.createIcons();
        });
    });

    // 7. Add Competency Modal Logic
    const addCompetencyBtn = document.getElementById("addCompetencyBtn");
    const addModal = document.getElementById("addCompetencyModal");
    const closeAddModal = document.getElementById("closeAddModal");
    const cancelAdd = document.getElementById("cancelAdd");
    const addForm = document.getElementById("addCompetencyForm");

    if (addCompetencyBtn && addModal) {
        ...
        if (closeAddModal) closeAddModal.addEventListener("click", closeModalFunc);
        if (cancelAdd) cancelAdd.addEventListener("click", closeModalFunc);
        
        // Close on overlay click
        addModal.addEventListener("click", (e) => {
            if (e.target === addModal) closeModalFunc();
        });
        ...
        if (addForm) {
            addForm.addEventListener("submit", async (e) => {
                ...
                submitBtn.innerHTML = '<i data-lucide="loader-2" class="animate-spin"></i> <span>Saving...</span>';
                if (typeof lucide !== "undefined") lucide.createIcons();

                const formData = new FormData(addForm);
                formData.append('action', 'add'); // Explicitly set action

                try {
                    const response = await fetch('backend/competency_action.php', {
                        method: 'POST',
                        body: formData
                    });
    ...
    // 8. Edit / Delete Logic
    const editModal = document.getElementById("editCompetencyModal");
    const editForm = document.getElementById("editCompetencyForm");
    ...
    document.querySelectorAll(".edit-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            const name = row.querySelector(".comp-name").textContent;
            const desc = row.querySelector(".comp-desc").textContent;

            // Populate form
            document.getElementById("edit_competency_id").value = id;
            document.getElementById("edit_category_id").value = categoryId;
            document.getElementById("edit_competency_name").value = name;
            document.getElementById("edit_competency_description").value = desc;
    ...
    if (document.getElementById("closeEditModal")) document.getElementById("closeEditModal").addEventListener("click", closeEditFunc);
    if (document.getElementById("cancelEdit")) document.getElementById("cancelEdit").addEventListener("click", closeEditFunc);
    if (editModal) {
        editModal.addEventListener("click", (e) => {
            if (e.target === editModal) closeEditFunc();
        });
    }
    ...
    if (editForm) {
        editForm.addEventListener("submit", async (e) => {
            ...
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i data-lucide="loader-2" class="animate-spin"></i> <span>Updating...</span>';
            if (typeof lucide !== "undefined") lucide.createIcons();
    ...
    document.querySelectorAll(".delete-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "This competency will be permanently removed.",
                icon: 'warning',
