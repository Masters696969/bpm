...
try {
    ...
    if ($res === false) {
        throw new Exception('Could not reach Finance Laptop (10.112.107.207): ' . $error . '. Please ensure both laptops are on the same network.');
    }

    $decoded = json_decode($res, true);
    if (!is_array($decoded) || !isset($decoded['success'])) {
        $rawLog = "Finance Response (HTTP $httpCode): " . $res;
        error_log($rawLog);
    ...
    if ($decoded['success']) {
        ...
        if (isset($decoded['finance_ref'])) {
            ...
            $updateRef = $conn->prepare("UPDATE compensation_period SET budget_finance_ref = ? WHERE period_id = ?");
            $updateRef->bind_param("si", $ref, $periodId);
            $updateRef->execute();
            $updateRef->close();
        }
    } else {
