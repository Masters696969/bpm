...
if (!isset($_SESSION['username'])) {
    respond(false, ['message' => 'Unauthorized access.'], 401);
}

$accountId = $_SESSION['account_id'] ?? $_SESSION['user_id'] ?? $_SESSION['AccountID'] ?? null;

if (!$accountId) {
    respond(false, ['message' => 'Unauthorized access. Missing admin account ID.'], 401);
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!in_array($leave['Status'], ['PENDING', 'APPROVED_BY_OFFICER'], true)) {
            throw new Exception("Only pending leave requests can be decided by admin.");
        }
        ...
        $updateSql = "
            ...
            WHERE LeaveRequestID = ?
              AND Status IN ('PENDING', 'APPROVED_BY_OFFICER')
            LIMIT 1
        ";
