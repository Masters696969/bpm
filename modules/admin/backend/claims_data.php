...
if (!$accountId) {
    respond(false, ['message' => 'Unauthorized access. Missing admin account session.'], 401);
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (strtoupper(trim((string)$claim['Status'])) !== 'PENDING') {
            throw new Exception("Only pending claims can be updated by Admin.");
        }
