...
if ($action === 'stats') {
    ...
    // Total for disbursement (Approved/Pending Approval)
    $res = $conn->query("SELECT SUM(i.net_pay) as total FROM payroll_batch_items i INNER JOIN payroll_batches b ON i.batch_id = b.id WHERE b.status IN ('Pending Approval', 'Approved')");
    ...
    // Counts
    $res = $conn->query("SELECT COUNT(*) as cnt FROM payroll_batches WHERE status IN ('Pending Approval', 'Approved')");
    if ($res && ($row = $res->fetch_assoc())) {
...
if ($action === 'list_batches') {
    ...
    $status = ($filter === 'history') ? "'Disbursed', 'Archived', 'Finance Approved'" : "'Pending Approval', 'Approved'";
...
if ($action === 'disburse_batch') {
    ...
    try {
        $stmt = $conn->prepare("UPDATE payroll_batches SET status = 'Finance Approved' WHERE id = ? AND status IN ('Pending Approval', 'Approved')");
        ...
        if ($affectedRows > 0) {
            ...
            $conn->commit();
            respond(true, ['message' => 'Batch securely processed. AP Vouchers & GL entries generated. Ready for Payout.']);
        } else {
            $conn->rollback();
...
if ($action === 'archive_batch') {
    ...
    $stmt = $conn->prepare("UPDATE payroll_batches SET status = 'Archived' WHERE id = ? AND status IN ('Pending Approval', 'Approved')");
    $stmt->bind_param("i", $batchId);
