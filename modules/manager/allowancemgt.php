...
$sql = "SELECT ga.GradeAllowanceID as AllowanceID, sg.GradeLevel, sg.GradeName, at.AllowanceName, ga.Amount 
        FROM grade_allowances ga
        JOIN salary_grades sg ON ga.SalaryGradeID = sg.SalaryGradeID
        JOIN allowance_types at ON ga.AllowanceTypeID = at.AllowanceTypeID
        ORDER BY sg.GradeLevel ASC, at.AllowanceName ASC";
$result = $conn->query($sql);
$active_allowances = [];
if ($result) {
    while ($row = $result->fetch_assoc()) {
        $active_allowances[] = $row;
    }
}
...
<head>
  ...
  <link rel="stylesheet" href="../../css/allowancemgt.css?v=1.2">
...
<body>
  ...
  <main class="main-content">
    ...
    <div class="content-wrapper">
        <div class="content-card">
            <div class="card-header">
                <div class="card-header-left">
                    <h3 class="card-title">Current Active Allowances</h3>
                    <p class="card-subtitle">Active monetary allowances for different job grades.</p>
                </div>
            </div>
            <div class="card-body">
                <div class="data-table">
                    <table id="currentAllowanceTable">
                        <thead>
                            <tr>
                                <th>Job Grade</th>
                        ...
                        <tbody>
                            <?php foreach ($active_allowances as $setting): ?>
                            <tr data-id="<?php echo $setting['AllowanceID']; ?>">
                                <td><strong><?php echo htmlspecialchars($setting['GradeLevel']); ?></strong> / <span style="font-size:13px; color:var(--text-secondary);"><?php echo htmlspecialchars($setting['GradeName']); ?></span></td>
                                <td><?php echo htmlspecialchars($setting['AllowanceName']); ?></td>
                                <td style="color:var(--brand-green); font-weight:600;">&#8369;<?php echo number_format($setting['Amount'], 2); ?></td>
                            </tr>
                            <?php endforeach; ?>
                            <?php if(empty($active_allowances)): ?>
                            <tr><td colspan="3" style="text-align:center;">No active allowances found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="content-card" style="margin-top: 24px;">
            <div class="card-header">
                <div class="card-header-left">
                    <h3 class="card-title">Endorsed Allowance Proposals</h3>
    ...
    <div class="modal-overlay hidden" id="proposalActionModal">
      <div class="rem-dialog" style="max-width: 800px; width: 90%;">
        ...
        <div class="rem-header">
          ...
          <button type="button" class="rem-close" id="btnCloseProposalModal" title="Close">
            <i data-lucide="x"></i>
          </button>
        </div>
  ...
  <script src="../../js/allowancemgt.js"></script>
</body>
</html>
