...
<head>
  ...
  <link rel="stylesheet" href="../../css/allowance.css?v=1.2">
...
<body>
  ...
  <main class="main-content">
    ...
    <div class="content-wrapper">
          ...
          <div class="tab-panel active" id="allowances">
            <div class="section-header" style="margin-bottom: 24px; display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 16px; border-bottom: 1px solid var(--border-color);">
              ...
              <div class="comp-panel-actions" style="display: flex; gap: 12px;">
                ...
                <button class="btn btn-outline" id="btnTrackAllowanceStatus" style="border: 1px solid var(--border-color); color: var(--text-secondary);">
                  <i data-lucide="eye"></i>
                  <span>Track Status</span>
                </button>
                <button class="btn btn-primary" id="btnProposeAllowanceChange">
      ...
      <!-- Propose Allowance Change Modal -->
      <div id="proposeAllowanceModal" class="modal" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" style="max-width: 900px;">
          <form id="allowanceProposalForm">
            <div class="comp-modal-hero">
              <div class="comp-modal-hero-inner" style="padding: 12px 16px;">
                <div class="comp-modal-hero-icon" style="background: rgba(245, 158, 11, 0.2); width: 36px; height: 36px; color: #f59e0b;">
                  <i data-lucide="gift" style="width: 18px; height: 18px;"></i>
                </div>
                <div class="comp-modal-hero-text">
                  <h3 style="font-size: 16px;">Propose Allowance Adjustments</h3>
                  <p style="font-size: 11px;">Update standard non-taxable and taxable allowances by grade.</p>
                </div>
                <button type="button" class="rp-close-modal" id="closeProposeAllowanceModalBtn" title="Close">&times;</button>
              </div>
            </div>
            <div class="modal-body modal-form-premium" style="max-height: 50vh; overflow-y: auto;">
              <div class="table-responsive">
                <table class="comp-table editable-table" id="proposeAllowanceTable">
                  <thead>
                    <tr>
                      <th>Grade Level</th>
                      <?php foreach ($allowance_types as $type): ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <div class="input-with-symbol">
                          <span>&#8369;</span>
                          <input type="number" 
                                 class="table-input-premium allowance-prop-input"
                                 data-grade="<?php echo $grade['SalaryGradeID']; ?>"
                                 data-type="<?php echo $type['AllowanceTypeID']; ?>"
                                 data-original="<?php echo (int)$amount; ?>"
                                 value="<?php echo (int)$amount; ?>">
                        </div>
                      </td>
                      <?php endforeach; ?>
              ...
              <div class="form-group" style="margin-top: 20px;">
                ...
                <textarea id="allowanceProposalReason" rows="3" class="input-premium no-icon" placeholder="Explain the rationale behind these proposed allowance changes..." required></textarea>
              </div>
            </div>
            <div class="modal-footer-premium">
              <button type="button" id="cancelAllowanceProposeBtn" class="btn-cancel-premium">Cancel</button>
      ...
      <!-- Track Allowance Status Modal -->
      <div id="trackAllowanceModal" class="modal" aria-hidden="true" style="display: none;">
        <div class="modal-dialog" style="max-width: 800px;">
          <div class="comp-modal-hero">
            <div class="comp-modal-hero-inner">
              <div class="comp-modal-hero-icon" style="background: rgba(245, 158, 11, 0.2); color: #f59e0b;">
                <i data-lucide="eye"></i>
              </div>
              <div class="comp-modal-hero-text">
                <h3>Track Allowance Proposals</h3>
                <p>Monitor the status of your submitted allowance adjustments.</p>
              </div>
              <button type="button" class="rp-close-modal" id="closeTrackAllowanceBtn" title="Close">&times;</button>
            </div>
          </div>
          <div class="modal-body modal-form-premium" style="min-height: 400px; max-height: 70vh; overflow-y: auto;">
            <!-- Stage A: List of Batches -->
            <div id="allowanceStageAList">
                <div class="data-table-premium">
                    <table style="width: 100%; border-collapse: separate; border-spacing: 0 8px;">
                        ...
                        <tbody id="allowanceTrackingBody">
                ...
                <div class="pagination-premium" style="display: flex; justify-content: space-between; align-items: center; margin-top: 16px; padding: 0 8px;">
                    <div style="font-size: 13px; color: var(--text-secondary);">Showing <span id="allowancePageRange">0</span> of <span id="allowanceTotalEntries">0</span></div>
                    <div style="display: flex; gap: 8px;">
                        <button id="prevAllowancePage" class="btn-pagination" disabled><i data-lucide="chevron-left"></i></button>
                        <button id="nextAllowancePage" class="btn-pagination" disabled><i data-lucide="chevron-right"></i></button>
                    </div>
                </div>
            </div>

            <!-- Stage B: Stepper & Details -->
            <div id="allowanceStageBDetails" style="display: none;">
                <button type="button" class="btn btn-secondary" id="btnBackToAllowanceList" style="margin-bottom: 24px; padding: 6px 12px; font-size: 13px;">
                    <i data-lucide="arrow-left" style="width: 16px; height: 16px;"></i> Back to List
                </button>

                <div class="stepper-hero-premium" style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.05) 0%, rgba(59, 130, 246, 0.05) 100%); border-radius: 16px; padding: 32px 20px; border: 1px solid var(--border-color); margin-bottom: 32px; box-shadow: var(--shadow-sm);">
                    <div class="stepper-header" style="text-align: center; margin-bottom: 32px;">
                        <h4 id="allowanceStepperBatchTitle" style="font-size: 20px; font-weight: 800; color: var(--text-primary); margin-bottom: 8px; letter-spacing: -0.02em;">Batch: ----</h4>
                        <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
                            <i data-lucide="info" style="width: 14px; height: 14px; color: #f59e0b;"></i>
                            <p id="allowanceStepperBatchReason" style="color: var(--text-secondary); font-size: 13px; font-style: italic;"></p>
                        </div>
                    </div>

                    <div class="stepper-container" style="max-width: 650px;">
                        <div class="stepper-line">
                            <div class="stepper-line-fill" id="allowanceStepperLineFill" style="width: 0%; background: #f59e0b;"></div>
                        </div>

                        <div class="stepper-step allowance-step" id="allowanceStep1">
                            <div class="step-circle"><i data-lucide="check"></i></div>
                            <div class="step-label">Step 1</div>
                            <div class="step-title" style="font-size: 13px;">Submitted</div>
                            <div class="step-desc" id="allowanceStep1Desc">Analyst</div>
                        </div>

                        <div class="stepper-step allowance-step" id="allowanceStep2">
                            <div class="step-circle"><i data-lucide="search"></i></div>
                            <div class="step-label">Step 2</div>
                            <div class="step-title" style="font-size: 13px;">Under Review</div>
                            <div class="step-desc" id="allowanceStep2Desc">Pending</div>
                        </div>

                        <div class="stepper-step allowance-step" id="allowanceStep3">
                            <div class="step-circle"><i data-lucide="user-check"></i></div>
                            <div class="step-label">Step 3</div>
                            <div class="step-title" style="font-size: 13px;">Endorsed</div>
                            <div class="step-desc" id="allowanceStep3Desc">Supervisor</div>
                        </div>

                        <div class="stepper-step allowance-step" id="allowanceStep4">
                            <div class="step-circle"><i data-lucide="flag"></i></div>
                            <div class="step-label">Step 4</div>
                            <div class="step-title" id="allowanceStep4Title" style="font-size: 13px;">Finalized</div>
                            <div class="step-desc" id="allowanceStep4Desc">Pending</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="details-section-premium">
                    <div style="display: flex; align-items:center; gap: 10px; margin-bottom: 16px; border-bottom: 1px solid var(--border-color); padding-bottom: 10px;">
                        <i data-lucide="list-checks" style="color: #f59e0b; width: 20px; height: 20px;"></i>
                        ...
                        <table style="width: 100%; border-collapse: separate; border-spacing: 0 4px;">
                            <thead>
                                <tr style="background: rgba(0,0,0,0.02);">
                                    <th style="border-radius: 8px 0 0 8px; padding: 12px; font-size: 12px; text-transform: uppercase;">Grade Level & Type</th>
                                    <th style="padding: 12px; font-size: 12px; text-transform: uppercase;">Old Amount</th>
                                    <th style="border-radius: 0 8px 8px 0; padding: 12px; font-size: 12px; text-transform: uppercase; color:var(--brand-green);">Proposed Amount</th>
                                </tr>
                            </thead>
                            <tbody id="allowanceTrackDetailsTableBody"></tbody>
                        </table>
                    </div>
                </div>
            </div>
